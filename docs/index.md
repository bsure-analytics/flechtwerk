...
From there, dig into [typed attributes](concepts/typed-attributes.md) — the middle ground between bare dicts and a dataclass per processing step — the [task model](concepts/exactly-once.md) behind exactly-once delivery, [config topics](concepts/config-topics.md) as shared lookup tables, and the [MQTT extractor](guides/mqtt.md) that ACKs only once Kafka has the data.
