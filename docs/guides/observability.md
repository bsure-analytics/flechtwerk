...
| `message_processing_seconds` | Histogram | — | Time in a single `transform()` / `poll()` dispatch (excluding Kafka I/O). |
...
| `transactions_committed_total` | Counter | — | Kafka transactions successfully committed. *Transformer only.* |
