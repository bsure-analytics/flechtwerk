...
| `types.py` | `Config`, `Event`, `State` (`Record` subclasses) plus the `IncomingMessage` / `Message` dataclass envelopes (timestamps are real `datetime`, not millis). Hosts the non-exported `Stage` base shared by both stage kinds. |
