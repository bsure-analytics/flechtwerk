...
!!! note "Scaling Is the Replica Count"
    Extractor output is deliberately non-transactional (at-least-once) — the transformer's fencing primitive doesn't exist here. Instead, replicas of one `application_id` shard the *config set* between them via consumer-group leases; a single replica owns everything.

Instances join the `application_id` consumer group and lease the config topics' partitions as ownership **tokens**: each instance polls only the configs whose state key hashes onto a held token (`token_for` — the default partitioner's murmur2 math, computed consumer-side, so config-record *placement* stays irrelevant and Kafka UI writes keep working). The data plane is unaffected — every instance reads every config topic group-less into the global store, and `self.configs` exposes exactly that store. The rebalance protocol mirrors transformer tasks: the revoke callback is the handover barrier (cancel the in-flight cycle, flush changelog stragglers, wipe the local store), and the new owner re-reads the full changelog before polling — the changelog needs no partition alignment. An extractor's config topics must share one partition count (the token space, validated at startup); extra replicas beyond it are hot standbys. Delivery stays at-least-once: a zombie can regress a cursor around a rebalance — typically by one in-flight poll, at worst by the window until an evicted instance notices — a bounded re-import, not loss. Run MQTT-sourced extractors as one replica (no unsubscribe lifecycle yet — see [One Replica for Now](../guides/mqtt.md#one-replica-for-now)).
