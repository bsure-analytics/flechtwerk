...
- **State must be designed bounded.** Each state key's value is **one** changelog record, serialized whole on every commit — so it is subject to Kafka's record ceilings: the producer's `max_request_size` and the topic's `max.message.bytes`, both ~1 MiB by default. Growth-only state (an unpruned seen-set, an unbounded bucket) therefore has a deterministic death date. The framework does not guard or raise ahead of Kafka: the producer raises `MessageSizeTooLargeError` mid-transaction and the stage crashes, which is correct let-it-crash behavior but tells you nothing about *which* key. Bound the state instead — event-time pruning, a hard cap, stalest-first eviction — and watch [`flechtwerk_state_record_bytes`](../guides/observability.md#state-tasks-and-tokens) and its `_max_bytes` high-water mark to see the ceiling coming. Operators who need different limits may pre-create `<application_id>-changelog` themselves with custom topic configs; the framework treats an existing topic as success and adopts it.
